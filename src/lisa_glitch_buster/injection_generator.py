...
class InjectionGenerator:
    ...
    def optimal_snr(signal, sens_mat=SENSITIVITY_MATRIX):
        ...
        h_inner = (
            4
            * hf.df
            * np.real(np.sum(hf[:, 1:].conj() * hf[:, 1:] / sens_mat[:, 1:]))
        )
    ...
    def plot_prior_samples(n=1000):
        ...
        hist, _ = hist_collection(
            snrs, bins=np.geomspace(5, 300, 50), ax=axes[1]
        )
        if (hist[0] + hist[-1]) > 0:
        ...
        if (hist[0] + hist[-1]) > 0:
            warnings.warn(
                "End time distribution goes beyond the desired limits..."
            )
