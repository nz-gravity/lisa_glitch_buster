...
def plot_corner(chains, **kwargs):
    ...
    assert nChains <= len(defaultColorsOrder), (
        "currently only supports up to "
        + str(len(defaultColorsOrder))
        + " chains"
    )

    # Check that each chain looks reasonable (2d shape)
    for i in range(nChains):
        assert len(chains[i].shape) == 2, "unexpected shape of chain %d" % (
            chains[i]
        )
    ...
    if deprecated_nContourLevels:
        warnings.warn(
            "nConfidenceLevels has been replaced by nContourLevels",
            DeprecationWarning,
        )
    ...
    confLevels = kwargs.pop("confLevels", (0.3173, 0.0455, 0.0027))
    ...
    if deprecated_ConfLevels:
        warnings.warn(
            "gaussianConfLevels replaced by sigmaContourLevels",
            DeprecationWarning,
        )
        confLevels = (0.6065, 0.1353, 0.0111)
    deprecated_ConfLevels = kwargs.pop("GaussianConfLevels", False)
    ...
    if (smoothingKernel != 0) and (not haveScipy):
        warnings.warn(
            "Scipy not installed. Curves will not be smoothed.", UserWarning
        )
        smoothingKernel = 0
    ...
    samplesMin = np.nanmin(
        np.array([np.nanmin(chains[k], axis=0) for k in range(nChains)]),
        axis=0,
    )
    samplesMax = np.nanmax(
        np.array([np.nanmax(chains[k], axis=0) for k in range(nChains)]),
        axis=0,
    )
            ...
            for j in range(nDim):  # column
                if j < i:
                    ...
                    if i == nDim - 1:
                        if paramNames is not None:
                            ax.set_xlabel(
                                paramNames[j], fontdict=customLabelFont
                            )
                    else:
                    ...
                    if j == 0:
                        if paramNames is not None:
                            ax.set_ylabel(
                                paramNames[i], fontdict=customLabelFont
                            )
                    else:
                    ...
                    if xTicks[j] is None:
                        ...
                        idx = np.where(
                            (tickLocs > LoHi[0]) & (tickLocs < LoHi[1])
                        )[0]
                    ...
                    for xLabel in ax.get_xticklabels():
                        ...
                        # render
                        def _mpx(self, x):
                            return mpl.text.Text.set_x(
                                self, x + self.custom_shift
                            )
                        ...
                        if _PYVER >= 3:
                            ...
                            xLabel.set_x = types.MethodType(
                                _mpx, xLabel, mpl.text.Text
                            )
                    ...
                    for yLabel in ax.get_yticklabels():
                        ...
                        # render
                        def _mpy(self, y):
                            return mpl.text.Text.set_y(
                                self, y + self.custom_shift
                            )

                        if _PYVER >= 3:
                            yLabel.set_y = types.MethodType(_mpy, yLabel)
                        else:
                            yLabel.set_y = types.MethodType(
                                _mpy, yLabel, mpl.text.Text
                            )
...
def __plot1d(
    ...
    if prior1d is not None:
        ...
        arr = np.linspace(
            prior1d[0] - 4 * prior1d[1], prior1d[0] + 4 * prior1d[1], 40
        )
        ax.plot(
            arr, norm.pdf(arr, prior1d[0], prior1d[1]), lw=1, color=priorColor
        )

    if ref1d is not None:
        hist1d, edges = np.histogram(ref1d, bins=bins, density=True)
